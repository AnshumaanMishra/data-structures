...
class BinarySearchTree{
    ...
    public:
        ...
        TreeNode* getInorderSuccessor(TreeNode* currentNode){
            if(rightChildExists(currentNode)){
                return returnMinimum(currentNode->_rightChild);
            }
            else{
                return findNearestLeftParent(currentNode->_rightChild, _rootTreeNode);
            }
        }
        ...
        TreeNode* findNearestLeftParent(TreeNode* passedNode, TreeNode* currentNode){
            if((leftChildExists(currentNode)) && (currentNode->_value > passedNode->_value)){
                ...
                return findNearestLeftParent(passedNode, currentNode->_leftChild);
            }
            if(rightChildExists(currentNode) && (currentNode->_value < passedNode->_value)){
                return findNearestLeftParent(passedNode, currentNode->_rightChild);
            }
            return currentNode;
        }

        TreeNode* inorderSuccessor(int value){
            return getInorderSuccessor(search(value, _rootTreeNode));
        }
...
int main(){
    ...
    printf("\nInorder Succesor of 1 = %d\n", tree1.inorderSuccessor(1));
    printf("\n");
}